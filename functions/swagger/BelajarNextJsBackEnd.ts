...
export class BelajarNextJsBackEndClient {
     ...
     * @return Success
     */
    brands(search: string | undefined): Promise<Brand[]> {
     ...
     * @return Success
     */
    updateBrand(id: string, body: Brand | undefined): Promise<void> {
     ...
     * @return Success
     */
    productsAll(): Promise<ProductDataGridItem[]> {
    ...
    protected processProductsAll(response: Response): Promise<ProductDataGridItem[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
     ...
     * @return Success
     */
    products(id: string): Promise<Product> {
